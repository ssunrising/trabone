...
tridiagonal matrix, or of a real, symmetric matrix previously reduced by tred2 ¡ì11.2. On
...
In either case, the kth column of z returns the normalized eigenvector corresponding to d[k].*/
{
	...
	for (i=2;i<=n;i++) 
		e[i-1]=e[i]; //Convenient to renumber the elements of e. e[n]=0.0;
	for (l=1;l<=n;l++) 
	{
		iter=0;
		do 
		{
			...
			{
				if (iter++ == 30) 
					//MessageBox(_T("Too many iterations in tqli"),NULL,MB_OK);
